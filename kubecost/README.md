...
| `mcp.enabled`                                 | Boolean override for deploying the MCP subchart. When omitted, follows `aggregator.enabled`                                                                         | omitted (follows `aggregator.enabled`)                |
...
- `mcp.enabled` is omitted from the chart defaults. Helm then uses `aggregator.enabled` to decide whether to load the subchart, so existing agent-only installs (`aggregator.enabled: false`) skip MCP on upgrade without a new flag. Set `mcp.enabled: true` or `false` to override.
...
- The MCP server reads cost data through the Kubecost aggregator service, so `mcp.enabled` is left unset and the subchart follows `aggregator.enabled`: agent-only installs get no MCP server, and no new flag is needed to keep it that way. To run MCP against an aggregator deployed elsewhere, set **both** `mcp.enabled: true` and `mcp.config.kubecostApiBaseUrl` — repointing the URL alone is not enough, because with `aggregator.enabled: false` the subchart is never rendered. If Kubecost is deployed with SAML or OIDC authentication, the MCP server needs its own OIDC configuration under `mcp.config.oidc`; when it has its own authentication, consider setting `mcp.config.kubecostApiPort` to `9008` to bypass Kubecost's API authentication. Note the aggregator Service only exposes `9008` when `saml.enabled` or `oidc.enabled` is true.
